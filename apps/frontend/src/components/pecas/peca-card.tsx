...
export function PecaCard({ peca, mostrarArteiro = true }: { peca: PublicoPecaResumo; mostrarArteiro?: boolean }) {
  ...
  return (
    <Link href={`/pecas/${peca.id}`} className="group flex flex-col text-foreground">
      <div className="relative aspect-peca overflow-hidden border border-foreground/[.18] bg-muted">
        ...
        {peca.totalImagens > 1 && (
          <span className="absolute right-2 top-2 bg-foreground/80 px-1.5 py-0.5 text-xs font-semibold text-background">
